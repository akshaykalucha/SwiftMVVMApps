        ...
        VStack(spacing: 10){
            ...
            VStack(spacing: 5) {
                ForEach(0..<6) { index in
                    HStack {
                        Text("\(index + 1)")
                        if dm.currentStat.frequency[index] == 0{
                            Rectangle()
                                .fill(Color.wrong)
                            ...
                            if let maxValue = dm.currentStat.frequency.max() {
                                Rectangle()
                                    .fill((dm.tryIndex == index && dm.gameOver)
                                          ? Color.correct
                                          : Color.wrong)
                                    .frame(width: CGFloat(dm.currentStat.frequency[index]) / CGFloat(maxValue) * 210, height: 20)
                                    .overlay(
                                        Text("\(dm.currentStat.frequency[index])")
                                            .foregroundColor(.white)
